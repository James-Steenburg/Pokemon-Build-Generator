...
Trainer Battle:
    TrainerBattle.start(:YOUNGSTER, "Joey")
```

### Mix or Modify Teams

To partially randomize a team, use the following format:
...
The same syntax can be used to override PBS team sizes for a specific generated_teams_criterion.
This example takes the ANY_SIX criterion, but only generates three random pokemon.

```text
Script:
    RTG.start(
        RTG.random(:ANY_SIX, 3),
        level: 30,
        exclude_species: true
    )
